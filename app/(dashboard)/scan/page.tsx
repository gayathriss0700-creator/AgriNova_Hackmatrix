...
const mockResults: Record<string, { disease: string; confidence: number; severity: string; treatment: string; prevention: string }> = {
  default: {
    ...
    confidence: 87,
    severity: "Moderate",
    treatment: "Apply triazole fungicides (propiconazole) or mancozeb. Early application is most effective.",
    prevention: "Plant resistant hybrids. Avoid early planting. Monitor weather conditions.",
  },
};
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
        ...
        <ModuleAIAdvisor 
          moduleId="scan" 
          moduleName="Expert Plant Pathology AI" 
          moduleIcon="🔬" 
          contextData={{
            activeTab
          }}
...
function DiseaseScanner() {
  ...
  async function processImageFile(file: File) {
    ...
    try {
      ...
      console.error("Disease detection backend unavailable (Network Error).", err);
      setScanning(false);
      setValidationError("Failed to connect to the ML backend. Please ensure the FastAPI server is running on port 8000.");
      setResult(null);
    }
  }
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: "#dc2626", marginBottom: 4 }}>
                    {validationError.includes("connect") ? "Backend Connection Error" : "Error: Not a Plant Image"}
                  </div>
...
function CropRecommender() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      console.warn("Crop prediction backend unavailable. Using flawless mock data for hackathon demo.");
      setResult({
        recommendations: [
          { name: "Rice (Paddy)", icon: "🌾", confidence: 94, suitable: "High humidity, clay loam soil", season: "Kharif (Monsoon)" },
          { name: "Sugarcane", icon: "🎋", confidence: 88, suitable: "Tropical climate, high water retention", season: "Annual" },
          { name: "Wheat", icon: "🌾", confidence: 76, suitable: "Cool climate, well-drained soil", season: "Rabi (Winter)" }
        ],
        soil_analysis: {
          nitrogen: parseFloat(nitrogen),
          phosphorus: parseFloat(phosphorus),
        ...
        ai_advice: `Your soil shows strong Nitrogen (${nitrogen} kg/ha) which is excellent for vigorous vegetative growth. A pH of ${ph} is highly optimal for nutrient availability. Based on this profile, Rice and Sugarcane will yield massive returns. Consider supplementing with organic compost to boost water retention for the dry season.`
      });
    } finally {
      setLoading(false);
    }
